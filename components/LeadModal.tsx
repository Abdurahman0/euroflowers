...
export default function LeadModal({ lead, onClose, onStatus }: { lead: Lead; onClose: () => void; onStatus: (st: LeadStatus) => void }) {
  ...
  const Row = ({ k, v, accent }: { k: string; v: string; accent?: boolean }) => (
    <div className="flex justify-between gap-3.5 border-t border-white/20 px-4 py-3 first:border-t-0">
      <span className="text-[13px] text-white/60">{k}</span>
      <span className={clsx("text-right text-[13px] font-semibold", accent && "font-extrabold")} style={accent ? { color: "var(--accL)" } : undefined}>{v}</span>
    </div>
  );
    ...
    <Modal onClose={onClose}>
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="min-w-[140px] flex-1">
          ...
          <div className="text-[13px] text-white/65">{lead.customer_detail?.masked_phone || "telefon yo'q"}</div>
        </div>
      ...
      <div className="mt-4 rounded-2xl border border-white/25">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {ACTIONS.map((st) => (
          <button key={st} onClick={() => onStatus(st)} className="min-w-[100px] flex-1 rounded-xl border-[1.5px] border-white/35 py-2.5 text-[13px] font-bold text-white" style={lead.status === st ? { background: "var(--acc)", borderColor: "var(--acc)" } : undefined}>
            {STATUS_LABEL[st]}
          </button>
        ))}
      </div>
