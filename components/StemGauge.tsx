"use client";
import { gaugeOf, stems } from "@/lib/inventory";
...
export default function StemGauge({
  ...
  batch: Pick<StockBatch, "remaining_stems" | "received_stems">;
  segments?: number;
  compact?: boolean;
}) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-[12px] font-bold tabular-nums" style={{ color: g.tone === "empty" ? "var(--muted)" : "var(--text)" }}>
          {stems(batch.remaining_stems)} <span className="font-medium" style={{ color: "var(--muted)" }}>/ {stems(batch.received_stems)}</span>
        </span>
        <span className="text-[12px] font-bold tabular-nums" style={{ color: g.hue }}>{pctText}%</span>
      </div>
