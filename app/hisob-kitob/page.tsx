                  ...
                  <FragmentRows key={sale.history_id} open={openCat === sale.history_id}
                    row={
                      <tr onClick={() => setOpenCat(openCat === sale.history_id ? null : sale.history_id)} className="cursor-pointer border-t transition-colors hover:bg-[var(--hover)]" style={{ borderColor: "var(--line2)" }}>
                        ...
                        {/* ⚠️ QARZDAN kelgan sotuvda bu sana — TO'LOV kuni, gul chiqqan kun EMAS.
                            Belgi `paid_from_debt` dan (jonli javobda bor; OpenAPI'da e'lon qilinmagan). */}
                        <td className="px-2 py-2.5 text-right tabular-nums">{fmtDate(sale.sold_at)}{sale.paid_from_debt && <span className="ml-1 rounded-full px-1.5 py-px text-[10px] font-bold align-middle" style={{ background: "color-mix(in srgb, var(--acc) 16%, transparent)", color: "var(--acc)" }} title="Qarzdan to'langan — bu sana TO'LOV sanasi, gul avvalroq chiqqan">qarzdan</span>}<div className="text-[11px]" style={{ color: "var(--muted)" }}>{sale.quantity} ta{sale.flower_stems ? ` · ${sale.flower_stems} gul` : ""}</div></td>
